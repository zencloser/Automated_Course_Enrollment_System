...
def update_progress():
    ...
    # Fetch existing grade BEFORE updating so we can log history correctly
    existing = query("SELECT grade FROM Progress WHERE student_id=%s AND course_id=%s", (sid, cid))
    ...
    # Manually log to GradeHistory — covers both first-insert and update cases.
    # ON DUPLICATE KEY UPDATE does not fire BEFORE UPDATE triggers on all MySQL configs,
    # so we insert the history row explicitly whenever the grade actually changes.
    if old_grade is None or float(old_grade) != float(new_grade):
        query("""
            INSERT INTO GradeHistory(student_id, course_id, old_grade, new_grade, changed_by, reason)
            VALUES (%s, %s, %s, %s, %s, %s)
        """, (sid, cid, old_grade, new_grade, 'system', 'Grade updated via portal'), fetch=False)

    cert = query("SELECT cert_number FROM Certificates WHERE student_id=%s AND course_id=%s", (sid, cid))
